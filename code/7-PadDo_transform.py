...
attendance_count = (
        ...
        | beam.ParDo(SplitRow())
        # | 'Split rows using lambda function' >> beam.ParDo(lambda element: [element.split(',')])
        | beam.ParDo(FilterAccountsEmployee())
        | beam.ParDo(PairEmployees())
        | 'Group ' >> beam.GroupByKey()
        | 'Sum using ParDo' >> beam.ParDo(Counting())
        | beam.io.WriteToText('../data/pardo_transformed')
)
p1.run()

# Sample the first 20 results, remember there are no ordering guarantees.
# !{('head -n 20 data/output_new_final-00000-of-00001')}
print(os.system('cat ../data/pardo_transformed*'))
